...
function buildErgebnisseTable(subTab, rows, canEdit) {
  ...
  for (var i = 0; i < rows.length; i++) {
    var rr = rows[i];
    var ergebnis = rr.fmt === 'pkt' ? fmtPunkte(rr.resultat) : (rr.fmt || (subTab === 'sprungwurf' ? 'm' : '')) === 'm' ? fmtMeter(rr.resultat) : fmtTime(rr.resultat, (rr.fmt === 's' || (!rr.fmt && subTab === 'sprint')) ? 's' : (rr.fmt === 'min_h' ? 'min_h' : undefined));
