...
## v1570
- **RaceResult: Events mit mehreren Kategorien im selben Wettkampf korrekt zugeordnet.** Kategorie ist keine Eigenschaft der Veranstaltung, nur ein Import-Filter für die Disziplin-Auswahl – ein Event kann z.B. einen "5 km-Lauf" (Straße) UND einen "5.000 m Bahnlauf der Asse" (Bahn) kombinieren. Bisher (seit v1496) matchte "Bahnlauf" beim Straße-Import fälschlich auf "5km" statt "5000m Bahn", da Bahn-Disziplinen aus dem Kandidatenpool gefiltert waren. Signalisiert der Contest-/Listenname jetzt explizit eine andere Kategorie ("bahn"/"halle"/"indoor"/"cross"/"trail"), wird der Kandidatenpool nur für diese Auflösung erweitert, mit deutlichem Bonus für die signalisierte Kategorie (`rrBestDisz` neuer optionaler Parameter `preferNames`) – verhindert, dass ein zahlenbasierter Treffer in der falschen Kategorie knapp gewinnt. Verifiziert am 43. Dr. Ernst van Aaken Gedächtnislauf: Guido Wender/Frank Schramm liefen den Bahnlauf (18:15/18:36), wurden aber als "5km" statt "5000m Bahn" importiert.
- **Workflow-Hinweis:** eine Veranstaltung muss dafür nicht zweimal angelegt werden – zwei Import-Durchläufe (einmal Kategorie „Straße", einmal „Bahn") auf dieselbe, dabei automatisch wiedererkannte Veranstaltung sind der vorgesehene Weg.
...
## v1565
- **Mehrkämpfe (z.B. Dreikampf) werden importiert – neues Ergebnisformat „Punkte".** Der leichtathletik.de-Importer las nur `CurrentList`-Seiten; die Gesamtwertung eines Mehrkampfs steht aber als eigene `MultiResultList` („Dreikampf Kinder M8", Ergebnis = Punkte) und wurde übersprungen – nur die Einzeldisziplinen kamen an. Jetzt werden auch diese Listen gelesen: Disziplin = Wettkampfbezeichnung ohne Klassenzusatz, AK aus dem Listentitel (die Mehrkampfliste hat keine AK-Spalte). Die Struktur richtet sich beim ersten API-Aufruf selbst ein: Kategorie „Mehrkampf" (`mehrkampf`, Format `pkt`, absteigend sortiert) mit der Disziplin „Dreikampf"; die Kategorie wird der Kategoriegruppe von „Bahn" hinzugefügt, damit sie beim Bahn-Import zur Auswahl steht. Das Format „Punkte" ist in allen Formatauswahlen im Admin wählbar und wird überall als „649 Pkt." angezeigt (neue Helfer `fmtErgebnis()`, `fmtPunkte()`, `fmtHoeherBesser()` in `09_utils.js`); PB-Ermittlung und Zeitstrahl werten höhere Punkte als besser. Weitere Mehrkämpfe (Vierkampf, Siebenkampf …) lassen sich unter Admin → Disziplinen in der Kategorie „Mehrkampf" anlegen.

## v1564
- Fix RaceResult: Name/Jahrgang/Geschlecht/AK-Kürzel werden nicht erkannt, wenn RaceResult sie in Formel-Funktionen wie `nau([ES_Name])` einwickelt (statt nacktem `ES_Name`) – die bisherigen Prüfungen verlangten exakte Gleichheit (`f==='es_jahrgang'`), die am Wrapper abprallt. `iName` blieb dann auf dem hartcodierten Default-Index stehen (zufällig die Startnummer) statt auf einer Fehlermeldung. Alle vier `ES_`-Feldprüfungen (Name/Jahrgang/Geschlecht/AKAbkürzung) jetzt als Substring-Check, in allen betroffenen Kalibrierungs-Blöcken. Verifiziert am GLOMB City Marathon Bremerhaven: Nina Mayer erscheint jetzt korrekt statt der Startnummer „625"
...
- leichtathletik.de: gestaffelte Abkühlzeit wie bei RaceResult – 3 h bei den ersten Versuchen statt durchgehend 20 h. Eine Teilnehmerliste, die beim ersten Blick fehlte, ist oft Stunden später da.
- leichtathletik.de: Läufe ohne Arbeit melden jetzt ebenfalls ihren Grund (`hinweis`), und das Panel schlüsselt „offen" auf – **offen im Fenster · vor dem Fenster · ohne Teilnehmerliste**. Wettkämpfe vor dem Rückblick-Fenster standen sonst als unerklärlicher Rest da, genau die Falle, die bei RaceResult zu Rückfragen geführt hat.
- **uitslagen.nl** braucht das meiste davon nicht – es hat keine Warteschlange, ein gestörter Lauf verliert also nichts, weil der nächste ohnehin alles neu abfragt. Übernommen sind der Backoff bei 429/503 und ein zweiter Versuch nach Pause, damit eine einzelne Störung nicht den ganzen Durchgang kostet.
- Alle drei Statuszeilen weisen gedrosselte Abrufe getrennt aus.
- Nicht übertragen, weil quellenspezifisch: `EventOver` (nur RaceResult), Ausweich-Listennamen (nur RaceResult kennt mehrere Listen je Wettkampf), `showResults=false` (kein Gegenstück), die Reduktion der Suchbegriffe (leichtathletik.de vergleicht Vereinsnamen, statt zu suchen – zusätzliche Begriffe kosten dort nichts).
...
- **Antwortet RaceResult mit 429, wird der Lauf langsamer** – der Abstand zwischen Abrufen wächst um je eine Sekunde bis maximal fünf und bleibt für den Rest des Laufs erhöht. Vorher lief er unbeirrt weiter und sammelte weitere Fehlschläge ein.
- **Fix: Listen ohne Ausweichmöglichkeit.** Deckt eine Liste alle Wettbewerbe ab (Contest 0), wurde genau dieser eine Name verwendet – antwortete er mit 404 (verwaiste Einträge kommen in den Configs vor), galt der ganze Wettkampf als „keine Liste abrufbar". Je Contest werden jetzt bis zu drei Namen in Rangfolge vorgehalten und der Reihe nach probiert. Beim Testwettkampf 401583 (AERS Zomerloop) schlug der bevorzugte Name dauerhaft fehl; mit dem Ausweichnamen läuft er durch – von neun geprüften Wettkämpfen sind damit alle neun erfolgreich statt acht.
- Der Lauf weist gedrosselte Wettkämpfe getrennt aus (`gedrosselt`).
...
## v1539
- **Die Wettkämpfe vor dem Rückblick-Fenster lassen sich jetzt einsehen und gezielt nachholen.** Bisher nannte das Panel nur ihre Zahl. Neuer Endpunkt `GET rr-events-alt` (Admin) und im Panel zwei Knöpfe: „Liste anzeigen" (Datum, Name mit Link zu RaceResult, Ort, Zahl der Versuche und der Grund, aus dem der Wettkampf offen blieb) und „Diese nachholen".
- **Nachholen läuft ohne Discovery** (`&nodiscovery=1&tage=365`). Das ist der Kern des Problems, das die Liste immer länger werden ließ: Jeder Lauf mit `&tage=N` sucht auch N Tage zurück und trägt dabei weitere alte Wettkämpfe ein, die beim engeren Regelfenster sofort wieder herausfallen. Wer mit 30 Tagen nachholt, hat danach mehr Altbestand als vorher. Der neue Knopf arbeitet nur ab, was schon bekannt ist.
- Der erklärende Text im Panel benennt diesen Zusammenhang, damit die Zahl nicht als Fehler gelesen wird.

## v1538
- **Fix RaceResult: Ergebniszeilen mit der Spalte `AnzeigeTitel` wurden stillschweigend verworfen.** Die Feldzuordnung kannte `AnzeigeName`, nicht aber `AnzeigeTitel` – in RaceResult-Vorlagen ein ebenso gängiger Name für die Anzeigespalte des Teilnehmers. Der Name blieb dadurch leer, und `funde()` wirft namenlose Zeilen weg: Der Verein passte, der Fund ging trotzdem verloren. Betroffen war rund jeder zwölfte Wettkampf der Stichprobe. Gegenprobe am Wettkampf 384240: vorher 0 Funde, jetzt 43. Zusätzlich erkannt werden `DisplayName`, `Teilnehmer` und `Participant`.
- **Neu: Rückfall für Ergebnislisten ohne Vereinsspalte.** Manche Listen führen statt des Vereins nur den Wohnort (`CITY`). Da jede Zeile bereits ein Treffer der serverseitigen Suche ist, wird ein Suchbegriff, der in einem der Rohfelder steht, jetzt als Vereinsangabe übernommen – aber nur, wenn er **aus mindestens zwei Wörtern** besteht. „TuS Oedt" ist eindeutig genug, „Oedt" allein wäre auch ein Wohnort und bliebe folgenlos. Verifiziert an einem Wettkampf ohne Vereinsspalte: mehrteiliger Begriff findet 4 Zeilen, einteiliger trotz 10 Rohtreffern keine.
- Die Suchergebnisse führen dafür die Rohwerte der Zeile mit (`_roh`); gespeichert wird davon nichts.

## v1537
- **Die Cron-URL bringt ihr Zeitbudget jetzt selbst mit** (`&sekunden=`, abgeleitet aus „Wettkämpfe pro Lauf" mit rund 5 s je Wettkampf, gedeckelt auf 900). Vorher galt die Vorgabe von 240 s: wer das Budget hochsetzte, dessen Lauf brach nach vier Minuten ab und ließ den Rest liegen – ohne dass die URL im Panel darauf hingewiesen hätte. Gilt für RaceResult und leichtathletik.de (uitslagen.nl braucht es nicht, dort ist ein Lauf ein Abruf).
- Der Hinweis am Feld „Wettkämpfe pro Lauf" nennt jetzt die Größenordnung für einen **täglichen** Cron: Auf Samstag und Sonntag entfallen rund zwei Drittel aller Wettkämpfe (gemessen über 28 Tage in DE+NL+BE: Sa 42, So 24, Fr 16 gegenüber 1–4 an Wochentagen). Ein Tageslauf muss das Wochenende in einem Zug tragen, ein stündlicher nicht.
...
- `GET rr-scan` versteht jetzt `&tage=N` und überschreibt damit das Fenster für einen einzelnen Lauf (max. 365). Gedacht zum Abarbeiten eines Rückstands, ohne die Einstellung ändern und zurückstellen zu müssen – analog zu `uits-scan`.
- Discovery und Warteschlange rechnen die Fenstergrenze jetzt an einer Stelle aus (`$grenze`), statt sie an zwei Stellen getrennt zu bilden; der Lauf gibt sie als `ab` mit aus.

## v1532
- **RaceResult-Scanner: Wettkämpfe werden jetzt nach RaceResults eigenem Abschluss-Kennzeichen geprüft statt nach einer Zeitregel.** Die Event-Config liefert ein Feld `EventOver` – am Wettkampftag `False`, spätestens am Folgetag `True`. Bisher hat der Scanner stattdessen geraten: jeder Wettkampf wurde **zweimal** im Abstand von mindestens 20 Stunden durchsucht, in der Annahme, die Liste sei dann vollständig.
- Daraus folgen drei Verbesserungen: (1) **Ein Durchgang genügt** – halber Aufwand je Wettkampf. (2) Wettkämpfe, die noch laufen, werden **gar nicht erst durchsucht** und kosten nur den einen Config-Abruf statt zusätzlich bis zu vier Suchabrufe. (3) Es entstehen keine Funde mehr mit vorläufigen Zeiten, die beim nächsten Lauf als zweiter Fund derselben Person aufgetaucht wären (der Fund-Schlüssel enthält die Zeit).
- **Die Warteschlange nimmt vergangene Tage zuerst** (`ORDER BY (datum < CURDATE()) DESC, …`). Bisher standen die Wettkämpfe von heute vorn – also genau die, die noch gar nicht abgeschlossen sind. Bei 227 bekannten Wettkämpfen und einem Budget von 60 belegten sie den Platz, ohne verwertbare Listen zu liefern, während der ältere Rückstand liegen blieb.
- Das Statuspanel trennt „offen" jetzt auf: **geprüft · noch nicht dran · warten auf Abschluss · ohne Ergebnisliste**. Die alte Zeile („227 offen · 0 geprüft") las sich, als hätte der Lauf nichts bewirkt, obwohl 58 Wettkämpfe erfolgreich gescannt waren und nur auf ihren zweiten Durchgang warteten. Die Zeile zum letzten Lauf nennt zusätzlich die Zahl der noch nicht abgeschlossenen Wettkämpfe.

## v1531
- **leichtathletik.de-Scanner** (neu): dritte proaktive Quelle, jetzt für Bahn und Halle. Das DLV-Ergebnisportal hat keine vereinsübergreifende Suche; der Scanner kombiniert deshalb zwei HTML-Seiten: **Discovery** über `GET /Competitions/CurrentPast?page=N` (absteigend nach Datum, ~25 Wettkämpfe je Seite ≈ 3 Tage) und je Wettkampf die **Teilnehmerliste** `GET /Competitions/Competitoroverview/{id}` – vollständig in einem Dokument, ohne Paginierung, mit Startnummer, Name, Verein, Landesverband, Jahrgang und den gemeldeten Bewerben.
- Die Trefferprüfung ist damit eindeutiger als bei RaceResult: der Verein steht in einer eigenen Spalte, es wird verglichen statt gesucht. Zusätzliche Suchbegriffe kosten hier folglich keine weiteren Abrufe.
- **Wichtig: Es ist eine Melde-, keine Ergebnisliste.** Ein Fund heißt „war gemeldet", nicht „hat ein Ergebnis" – Panel und Text sagen das so. Geprüft wird erst ab dem Wettkampftag; da sich die Meldeliste danach nicht mehr ändert, genügt ein erfolgreicher Abruf je Wettkampf (anders als bei RaceResult, wo Ergebnislisten nachwachsen).
- Neue Tabellen `la_events` (Warteschlange, Status `offen`/`fertig`/`ohne_liste`) und `la_funde`; neue Endpunkte `GET la-scan?token=…`, `GET la-scan-status`, `GET la-funde`, `POST la-funde`. Panel „🔎 Neue Meldungen bei leichtathletik.de" auf der Eintragen-Seite (Link und Import zielen auf die Ergebnisübersicht, nicht auf die Meldeliste), Konfiguration unter Admin → Quellen, Fortschrittsbalken wie bei den anderen beiden.
- Mehrtägige Wettkämpfe stehen in der Liste als Zeitraum („25 - 26.08.2026", „30.08. - 01.09.2026"). Das Datum wird deshalb positionell aus `<time>` gelesen und auf den **letzten** Tag gesetzt – erst dann ist die Veranstaltung gelaufen. Ohne das wären genau die mehrtägigen Meisterschaften ohne Datum geblieben und ihr Name durch den Zeitraum ersetzt worden.
- In der Namensspalte steht bei Wettkämpfen mit Zeitplan zusätzlich ein „CIS"-Link; Name und Ort werden deshalb aus dem jeweiligen `Details`-Link der Zelle gelesen, sonst hieße der Wettkampf „… Hochsprungmeeting Heilbronn CIS".
- Rücksichtsvoll konfiguriert: `ergebnisse.leichtathletik.de` liefert eine robots.txt mit `Disallow: /` (Vorlage eines Bad-Bot-Blockers). Der Scanner läuft deshalb streng seriell mit 1,5 s Abstand, mit erkennbarem User-Agent, Standard-Rückblick 14 Tage, Budget 40 Wettkämpfe je Lauf – und ist ab Werk ausgeschaltet.
- `Scanner::gruppiere()` toleriert jetzt Quellen ohne Zeit- und Platzangabe (die Meldeliste kennt beides nicht).

## v1530
- **Neuer Admin-Tab „🔎 Quellen".** Die beiden Scanner (RaceResult, uitslagen.nl) haben jetzt eine eigene Seite, statt unter „Einstellungen" zwischen Wartungsmodus, Farben, Login-Portal und GitHub-Integration unterzugehen. Dort gehören sie auch inhaltlich nicht hin: sie haben laufenden Betrieb – Fortschritt, Fundzahlen, Cron-URL, „Jetzt scannen" –, während „Einstellungen" reine Konfiguration ist.
- Die Seite trägt eine kurze Erklärung, was die Scanner tun und dass die Funde auf der Eintragen-Seite landen. Panels, Eingabefelder und Statusanzeige sind unverändert übernommen.
- Die Eingabe-Helfer (`_admCfg`, `_admRow`, `_admTextIn`, `_admNumIn`) liegen jetzt auf Modulebene und werden von beiden Seiten genutzt; in `renderAdminDarstellung()` entfallen die lokalen Doppelungen.
- **Fix: Speichern von einer Einstellungsseite ohne Login-Portal-Panel hätte die registrierten Apps gelöscht.** `saveAllSettings()` schrieb `login_portal_apps` bedingungslos aus den vorhandenen Zeilen – auf einer Seite ohne diese Liste also eine leere. Jetzt wird der Schlüssel nur gesendet, wenn das Panel überhaupt auf der Seite ist. Aus demselben Grund läuft die Farbvalidierung nur noch über Felder, die die jeweilige Seite tatsächlich enthält (sonst hätte sie das Speichern mit „Ungültiger Farbwert" blockiert).
- Fix: Die Whitelist für Admin-Deep-Links kannte `orte` nicht – `#admin/orte` landete auf der Benutzerseite. `quellen` und `orte` sind jetzt beide eingetragen.
